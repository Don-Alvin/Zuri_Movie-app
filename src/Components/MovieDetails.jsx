...
const MovieDetails = () => {
  ...
  if(movie){
    ...
    console.log(stars, director, writers)
    

    content = (
      <section key={movie.id}>
        <section className='w-full lg:py-10 lg:px-8'>
          <div className='relative rounded-lg lg:px-8' key={movie.id}>
            <ReactPlayer width="100%" url={`https://www.youtube.com/watch?v=${trailer[0].key}`} />
          </div>
          <article className='grid lg:flex gap-2 px-4 lg:px-8'>
            <div className='w-full lg:w-[60%] my-4 pr-6 text-gray-700 flex flex-col gap-2' key={movie.id}>
            ...
            <aside className='lg:w-[40%] flex flex-col lg:items-center gap-4'>
